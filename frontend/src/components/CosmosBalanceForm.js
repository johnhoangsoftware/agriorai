...
function CosmosBalanceForm() {
    ...
    const [chainId, setChainId] = useState("Oraichain-testnet")
    const [token, setToken] = useState("uorai")
    const [rpcEndpoint, setRpcEndpoint] = useState("https://testnet-rpc.orai.io")
    const [exponent, setExponent] = useState(1e6)
    const [tokenName, setTokenName] = useState("ORAI")
