...
import { DATA_SOURCE } from "../configs/var.config";
...
export const CosmosBalanceCard = forwardRef((props, ref) => {
    ...
    const buttonHandlerKeplrConnect = async () => {
        ...
        if (window.keplr) {
            ...
            // Unlock the wallet.
            await window.keplr.enable(DATA_SOURCE.chanId);

            // Use offlineSigner to get first wallet and public key.
            // Currently only first address is supported.
            const offlineSigner = await window.getOfflineSigner(DATA_SOURCE.chanId);
